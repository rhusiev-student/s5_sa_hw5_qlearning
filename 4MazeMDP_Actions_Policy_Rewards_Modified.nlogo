globals[
  ...
  quality-list-direction-mapping
]
...
to setup
 clear-all
  ...
  ask patches [
    set neighbor-quality [0.0 0.0 0.0 0.0] ;up right down left
  ]
  ...
  set total-reward []
  set quality-list-direction-mapping [0 90 180 270]
...
to set-random-heading
  let valid-patch false

  while [valid-patch = false] [
    set heading one-of [0 90 180 270]
    set valid-patch check-constraints [pxcor] of patch-ahead 1 [pycor] of patch-ahead 1
  ]

end

to perform-q-learning
  let max-epoch number-iterations
  let epoch 0
  let success false
  ...
  while [epoch < number-iterations][
    ...
    while [success != true and steps < max-steps][
      ...
      ask walkers [
        set steps steps + 1
        let current-epsilon eeepsilon * ((1 / steps) ^ epsilon-decay-rate)

        if-else (random-float 1.0) < current-epsilon  [
          ...
          let max-quality max [neighbor-quality] of patch-here
          let index position max-quality [neighbor-quality] of patch-here
          let optimal-direction item index quality-list-direction-mapping
          set heading optimal-direction

          if check-constraints [pxcor] of patch-ahead 1 [pycor] of patch-ahead 1 = false [
            set-random-heading
          ]
        ...
        let previous-patch patch-here

        fd 1

        let reward-increment discount * (max [neighbor-quality] of patch-here) + [reward] of patch-here
        set current-reward current-reward + reward-increment ;update reward

        let index position heading quality-list-direction-mapping ;update quality

        ask previous-patch [
          set neighbor-quality replace-item index neighbor-quality ( reward-increment + item index neighbor-quality)
        ]
